...
- [x] G2-1. **One-click lead conversion** — "Approve" button on lead does EVERYTHING:
...
- [x] G2-2. **Resident self-signup** — invite link flow:
...
- [x] G2-3. **Auto-generate intake URLs per property** — when property is created, auto-generate:
...
- [x] G2-4. **Dashboard "Share Intake Link" card** — prominent section with:
  - Copy link button (one tap)
  - QR code (treatment centers print/scan)
  - "Embed on website" code snippet (iframe or styled button)
- [x] G2-5. **Branded intake form** — org name + logo at top, property pre-selected if property-specific link, mobile-optimized
- [x] G2-6. **Property detail page** shows that property's specific intake link + QR

**Dashboard as command center:**
- [x] G2-7. **Record Payment inline modal** — opens from dashboard, not page navigation
- [x] G2-8. **Add Resident inline modal** — quick-add from dashboard
- [x] G2-9. **Outstanding balance drill-down** — tap outstanding amount → see who owes what → tap resident → record payment
- [x] G2-10. **Action items that DO things** — each action item has an inline action button, not just a link
- [x] G2-11. **Remove all hardcoded/mock data** — every number comes from real queries
- [x] G2-12. **Empty state that guides** — new org sees "Add your first house" → "Add your first resident" → "Create first invoice" progression

**Expense tracking (multi-card, per-house):**
- [x] G2-13. **Plaid Link end-to-end** — operator connects bank account, sees transactions
- [x] G2-14. **Card-to-house mapping** — each linked account/card can be assigned to a house (or "all houses")
...
- [x] G2-15. **Auto-categorization** — transactions auto-categorized (Maintenance, Supplies, Utilities, Food, etc.)
- [x] G2-16. **P&L per house** — Revenue (rent collected) - Expenses (categorized) = Profit, per house, per month
- [x] G2-17. **Manual expense entry** — for cash purchases, receipts, etc.

**Stripe payments end-to-end:**
- [x] G2-18. **Resident "Pay Now" flow** — resident taps Pay → Stripe Checkout → card/Apple Pay/Google Pay → webhook → payment recorded → ledger updated → dashboard updated
- [x] G2-19. **Stripe Connect onboarding** — operator connects Stripe account (for receiving payouts)
- [x] G2-20. **Fee configuration tested** — absorb fees vs pass to resident, verify math

**DocuSign end-to-end:**
- [x] G2-21. **Send envelope from template** — owner picks template + resident → DocuSign sends
- [x] G2-22. **Embedded signing** — resident signs in-app (iframe)
- [x] G2-23. **Webhook completion** — signed doc saved, status updated, linked to resident profile

**Automation crons (make them actually work):**
- [x] G2-24. **Chore auto-rotation cron** — weekly rotation, residents notified
- [x] G2-25. **Random drug test selection cron** — picks residents per settings, notifies owner
- [x] G2-26. **Empty bed alert** — when bed vacated, calculate lost revenue, notify waitlist
- [x] G2-27. **Auto-invoice generation** — monthly invoice cron tested with real data
- [x] G2-28. **Weekly P&L digest** — automated report to owner

**Integration testing:**
- [x] G2-29. **Full operator flow** — signup → setup org → add house → add resident → collect rent → track expenses → view P&L
- [x] G2-30. **Full applicant→resident flow** — public form → lead → approve → documents → bed → paying resident
- [x] G2-31. **Full resident flow** — login → pay rent → view chores → message manager → sign document
- [x] G2-32. Clean build + all crons tested
