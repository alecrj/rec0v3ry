      ...
      <div className="p-4 space-y-6">
        <div className="min-h-[60vh] flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <h1 className="text-2xl font-bold text-zinc-800 mb-2">Payment Processing</h1>
            ...
            <Link
              href="/payments"
              className="w-full px-4 py-3 border border-zinc-200 text-zinc-600 rounded-lg font-medium hover:bg-zinc-100/40 flex items-center justify-center"
            >
              Back to Payments
            </Link>
          </div>
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/payments" className="p-2 hover:bg-zinc-100 rounded-lg transition-colors">
          <ArrowLeft className="h-5 w-5 text-zinc-400" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-zinc-800">Make a Payment</h1>
      ...
      {checkoutCanceled && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-zinc-600">Payment was canceled. Please try again.</p>
        </div>
      )}
      ...
      {invoices.length === 0 ? (
        <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-6 text-center">
          ...
          <p className="font-medium text-zinc-800">No outstanding invoices</p>
          <p className="text-sm text-zinc-400 mt-1">You&apos;re all caught up!</p>
        </div>
      ) : (
        <>
          <div className="bg-white rounded-lg border border-zinc-200 p-6">
            <h2 className="text-lg font-semibold text-zinc-800 mb-4">Select Invoices to Pay</h2>
                  ...
                  <label
                    ...
                    className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer transition-colors ${
                      isSelected
                        ...
                        : "border-zinc-200 hover:bg-zinc-100/40"
                    }`}
                  >
                    <input
                      type="checkbox"
                    ...
                    <p className="font-bold text-zinc-800">${amount.toFixed(2)}</p>
                  </label>
                );
              })}
            </div>
          ...
          <div className="bg-white rounded-lg border border-zinc-200 p-6">
            <div className="flex items-center gap-3 p-3 border border-zinc-200 rounded-lg">
              ...
              <div className="flex-1">
                <p className="font-medium text-zinc-800">Card, ACH, Apple Pay, Google Pay</p>
          ...
          <div className="bg-white rounded-lg border border-zinc-200 p-6">
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-zinc-800">{selectedInvoices.length} selected</span>
              </div>
              <div className="pt-3 border-t border-zinc-200 flex justify-between">
                <span className="font-semibold text-zinc-800">Total</span>
                <span className="text-xl font-bold text-zinc-800">${selectedTotal.toFixed(2)}</span>
              </div>
            </div>
          </div>
